...
export default function Dashboard() {
  ...
  const traerConteoNotasB = async (idMateria) => {
    setCargando(true);
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_CLIENT_URL}/reportes/notas/contador_notas/b/${
        idMateria || 1
      }`
    );
    console.log(res.data);
  ...
  const traerDocenteA = async (materiaId, division = "A") => {
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_CLIENT_URL}/gestion/docentes/usuario/${
        materiaId || 1
      }/${division}`
    );
